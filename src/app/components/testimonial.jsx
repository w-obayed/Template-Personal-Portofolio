...
export default function Testimonial() {
  ...
  const { data: testimonial } = useService("testimonial");
  ...
  return (
    <div
      className="bg-[#11081b] w-full px-4 md:px-0 lg:pt-20"
    >
      {/* Sticky Section */}
      <div className="min-h-150 py-12 md:py-24 flex flex-col justify-center max-w-7xl mx-auto">
        <div className="w-full">
          <Swiper
            ...
            onInit={() => requestAnimationFrame(setEqualHeight)}
            onResize={() => requestAnimationFrame(setEqualHeight)}
            onBreakpoint={() => requestAnimationFrame(setEqualHeight)}
            onSlidesUpdated={() => requestAnimationFrame(setEqualHeight)}
          ...
          >
            {(Array.isArray(testimonial) ? testimonial : []).map((item, index) => (
              <SwiperSlide key={item.id} className="h-auto flex transition-transform duration-300">
                <div ref={(el) => (cardRefs.current[index] = el)} className="relative w-full h-full card p-6 md:p-10 rounded-3xl bg-zinc-900/40 border border-zinc-800 hover:border-zinc-700 transition-colors flex flex-col">
                  ...
                  <div className="flex items-center justify-between mb-5">
                    ...
                    <div className="flex items-center gap-3">
                      <div className="w-12 h-12 rounded-full font-family-heading overflow-hidden shrink-0 ring-2 ring-zinc-700 bg-zinc-800 flex items-center justify-center text-zinc-400 text-xs font-bold">
                        {getInitials(item.name)}
                      </div>

                      <div>
                        <p className="text-white font-family-heading font-bold text-base leading-tight tracking-wide">
                          {item.name}
                        </p>
                         <p className="text-sm font-semibold mt-0.5 text-orange-400">
                          {generateStars(item.number_of_star)}
                        </p>
                      </div>
                    </div>

                    {/* Quote Icon */}
                    <div className="shrink-0 w-12 h-7.75  flex items-center justify-center">
                  ...
                  {/* Text */}
                  <div className="grow">
                    <p className="text-sm md:text-base leading-relaxed text-zinc-400 italic font-family-description">
