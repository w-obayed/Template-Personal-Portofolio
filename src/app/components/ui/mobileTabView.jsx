      ...
      <div className="flex flex-wrap gap-2">
        {slides.map(({ phase }, i) => (
          <button
            key={phase.id}
            onClick={() => onSelect(i)}
            className={`px-4 py-2 font-family-heading rounded-xl text-[18px] font-normal text-nowrap shadow-xl/20 transition-all duration-500 backdrop-blur-md ${i === currentIndex
          ...
          <div className="relative overflow-hidden rounded-3xl bg-[radial-gradient(ellipse_at_top,rgba(250,250,250,0.3)_0%,rgba(0,0,0,1)_40%)] backdrop-blur-xl p-6 lg:p-8 shadow-[0_0_50px_rgba(59,130,246,0.25)]">
            ...
            <div  className="relative z-10">
              <div className="flex items-start justify-between gap-4 mb-4">
                <span className="shrink-0 font-family-heading rounded-full text-2xl text-white/60">
                  {item.title}
                </span>
                <span className="shrink-0 font-family-description rounded-full text-sm text-white/60">
                  {item.duration}
                </span>
              </div>
              <p className="max-w-[90%] text-sm font-family-description lg:text-base leading-relaxed text-white/60">
                    {item.description}
              </p>
            </div>
          </div>
